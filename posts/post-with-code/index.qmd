...
# Identificación de corto plazo

$$
B_{0}y_t = B_{1}y_{t-1}+\cdots+B_{p}y_{t-p}+w_{t}
$$ {#eq-1}

Donde $\epsilon_{t}$ es el término del error estructural con las siguientes características:

1.  Media cero

2.  No correlación serial

3.  Homocedástica incondicional

$B_{0}$ es una matriz nosingular que denota las interacciones contemporáneas entre las variables incluidas en el vector $y_{t}$

La ecuación @eq-1 puede escribirse como:

$$
B(L)y_t = w_{t}
$$ {#eq-2}

Donde $B(L)=B_{0}-B_{1}L-B_{2}L^2-\cdots-B_{p}L^p$ es el polinomio de rezagos autoregresivos.

La matriz de varianza y covarianza de los errores estructurales se obtiene:

$$
\mathbb{E}(w_{t}w_{t}^{´}) = \Sigma_{w}=I_{k}
$$ {#eq-3}

Implicando:

1.  Hay tanto shocks estructurales como variables en el VAR
2.  Los shocks estructurales son por definición incorrelacionados por tanto $\Sigma_{w}$ es una matriz diagonal
3.  Se normalizan la varianza de todos lo shocks estructurales a la unidad.

**Advertencia** Supongamos un VAR formado por precios y cantidades tanto los shocks de demanda como de oferta pueden afectar tales variables. Por tanto, hay un problema de que estos shocks tengan una interpretación económica porque los shocks que afectan los precios son shocks de precios que difícilmente podría interpretarse por sí solo de forma económica.

A partir de la ecuación @eq-1 podemos deducir que:

$$
B_{0}^{-1}B_{0}y_t = B_{0}^{-1}B_{1}y_{t-1}+\cdots+B_{0}^{-1}B_{p}y_{t-p}+B_{0}^{-1}w_{t}
$$ {#eq-4}

La ecuación @eq-4 es el modelo en su forma reducida. 
$$
y_t = A_{1}y_{t-1}+\cdots+A_{p}y_{t-p}+u_{t}
$$ {#eq-5} 

Noten que: $u_{t}=B_{0}^{-1}w_{t}$ lo que significa que los errores en su forma reducida son un promedio ponderado de los shokcs estructurales.

La estimación se hace sobre el modelo dado por la ecuación @eq-5.

**El problema:** *¿cómo habiendo estimado el modelo en su forma reducida obtenemos los shocks estructurales?*


**Solución:** 

1. La solución pasa por encontrar $B_{0}^{-1}$. 

2. Sabemos que: 
$$\Sigma_{u}=\mathbb{E}(u_{t}u_{t}^{´})= B_{0}^{-1}\mathbb{E}(w_{t}w_{t}^{´})B_{0}^{-1´}=B_{0}^{-1}\Sigma_{w}B_{0}^{-1´}=B_{0}^{-1}I_{w}B_{0}^{-1´}=B_{0}^{-1}B_{0}^{-1´}$$ {#eq-5}

3. Dado que $\Sigma_u$ es una matriz simétrica índica que existe $(k(k+1))/2$ ecuaciones independientes. Por tanto el sistema de ecuaciones no lineales dado por la ecuación @eq-5 se puede obtener a través de métodos numéricos con la condición que: *El número de elementos desconocidos en la matriz* $B_{0}^{-1}$ *no exceda el numero de ecuaciones independientes del sistema*. Lo que obliga ha poner restricciones sobre la matriz $B_{0}^{-1}$.

4. Las restricciones pueden ser del siguiente tipo
 i) Exclusión (se fuerza que cierto elementos tomen valores iguales a cero)
 ii) Proporcionalidad
 iii) Igualdad

5. Noten que $(k(k+1))/2$ es el máximo número de elementos en la matriz $B_{0}^{-1}$ que son libres (condición de orden) siendo ésta la tarea de identificación. 

6. Si la condición de orden es satisfecha existe una solución única en el sentido estadístico pero no en el sentido económico ello debido a que existen supuestos alternativos para la normalización siendo éstos:
  i) Estándar con base en: $\Sigma_{w}=I_{k}$ 
  ii) Dejar libres los valores de la diagonal de la matriz $\Sigma_{w}$ pero restringir los valores de la diagonal de la matriz  $B_{0}$ a la unidad, de manera que $w_{t}=B_{0}u_{t}$. Esto significa que se tendría en la estimación de  $B_{0}$  $(k(k-1))/2$ restricciones. 
  iii) Por convención la selección de alternativa lo dicta la naturaleza económica de los supuestos de identificación. Con base a ello uno escoge restringir $B_{0}^{-1}$ (primera alternativa) o restringir $B_{0}$ (segunda alternativa).
  iv) Las alternativas anteriores pueden resumirse con base en $B_{0}u_{t}=Cw_{t}$,
  donde $\Sigma_{w}=I_{k}$ lo que implica que: $\Sigma_{u}=B_{0}^{-1}CC^{´}B_{0}^{-1´}$
  de manera que si:
   A.   $C=I_{k}$ tenemos el caso estándar
   B.   Pero también se puede poner restricciones tanto en $B_{0}$ como en $C$. Sí las restricciones son sobre los elementos de la diagonal de la matriz $B_{0}$ los cuales se restringe a la unidad y $\Sigma_{w}=I_{k}$ se tendría $k^{2}+(k(k-1))/2$ restricciones sobre  $B_{0}$ y $C$. Asimismo, si los elementos diagonales de
tanto $B_{0}$ y $C$ están restringidos a la unidad y los elementos diagonales de $\Sigma_{w}$ se dejan sin restricciones, la condición de orden exige restricciones $k^{2}+(k(k+1))/2$, incluyendo las restricciones de normalización en las diagonales.
...
La fórmula de descomposición de Beveridge-Nelson se puede expresar de la siguiente manera en LaTeX:
...
\end{equation*}
```
donde $y_t$ es la serie temporal original, $\tau_t$ es el componente tendencial y $\phi_t$ es el componente cíclico.

Además, el componente tendencial $\tau_t$ se puede expresar como:

```{=tex}
\begin{equation*}
\tau_t = \sum_{j=-\infty}^{+\infty} h_j y_{t-j}
\end{equation*}
```
donde $h_j$ es la función de filtro de Hodrick-Prescott, y el componente cíclico $\phi_t$ se puede expresar como:
